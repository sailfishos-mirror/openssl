...
=head3 Deprecation of ASN1_OBJECT_new()

The function ASN1_OBJECT_new() has been deprecated in 4.0. The function is only
useful internally because the type is opaque and there exist no setter functions.
...
modifies custom "METHODS" (for example EVP_MD_meth_new(),
EVP_CIPHER_meth_new(), EVP_PKEY_meth_new(), L<RSA_meth_new(3)>,
